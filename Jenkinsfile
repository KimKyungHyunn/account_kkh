    ...
    stages {
        stage('Checkout') {
            steps {
                ...
                git branch: 'main', url: 'https://github.com/KimKyungHyunn/account_kkh.git'
            }
        }
        ...
        stage('Deploy and Service to Kubernetes') {
            steps {
                script {
                    ...
                    sh "kubectl apply -f ./yaml/account.yaml -n ${NAMESPACE}"
                }
            }
        }
        stage('Deployment Image to Update') {
            steps {
                script {
                    ...
                    sh "kubectl set image deployment/account-kkh account-kkh=${REGISTRY}/${IMAGE_NAME}:${IMAGE_TAG} --namespace=${NAMESPACE}"
                }
            }
        }
    }
